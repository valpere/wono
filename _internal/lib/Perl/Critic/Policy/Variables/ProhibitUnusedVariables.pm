...
Readonly::Scalar my $EXPL => q<Unused variables clutter code and make it harder to read>;
...
sub violates {
    ...
    foreach my $declaration ( @{$declarations} ) {
        ...
        # class::std has:
        #     my %var : ATTR(...)
        next DECLARATION if any { $_ eq 'ATTR' } @children;
    ...
    } ## end DECLARATION: foreach my $declaration ( @...)
...
sub _get_regexp_symbol_usage {
    ...
    } ## end foreach my $class ( qw{ PPI::Token::Regexp::Match...})

    return;
} ## end sub _get_regexp_symbol_usage
